...
const {
	...
	next,
	_sort
} = require('./public/js/method')
const {
	judgement
} = require('./public/js/fun')
...
router
	...
	.post('/login', async (req, res) => {
		...
		let {
			length
		} = await db(`select * from user where nick='${nick}'`)
		if (length) {
			...
			const {
				affectedRows
			} = await db(`insert into user values('${nick}','${password}','${new Date().getTime()}',0,true)`)
			if (affectedRows > 0)
				res.send({
					status: 3,
					message: '新注册',
...
io.on('connection', (socket) => {
	...
	socket.on('jiaodizhu', ({
		...
		if (room.round == 0 && !room.jiaodizhu[0].includes(undefined)) { //第一轮最后一次 叫地主轮完成
			const {
				length
			} = room.jiaodizhu[0].filter(each => each)
			if (length == 1) { //直接开始游戏，将地主牌派给地主
			...
			if (flag) {
				...
				if (!room.jiaodizhu[1].includes(undefined)) {
					...
					const re = [room.jiaodizhu[0][arr[0]], //0 2 => 1 => 0
			...
			next(room.member)
		if (i == (room.flag + 2) % 3 && room.round < 2) //0 1 2
			room.round++
		io.sockets.in(roomid).emit('renew', {
			roommsg
		})
